...
int lexeme_looks_punctuator(int ch1)
{
	switch (ch1) {
		...
		case '\'' : return (TOKEN_SEP_SINGLE_QUOTE);
		case '"'  : return (TOKEN_SEP_DOUBLE_QUOTE);
		case ';'  : return (TOKEN_SEP_SEMI_COLON);
		case ':'  : return (TOKEN_SEP_COLON);
		case '}'  : return (TOKEN_SEP_CLOSE_CURLY_BRACKETS);
		case '{'  : return (TOKEN_SEP_OPEN_CURLY_BRACKETS);
